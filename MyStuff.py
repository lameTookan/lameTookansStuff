...
def frame_text(text: str, *args, padding_horizontal: int = 4, padding_vertical = 4 ) -> str:
    """Creates a frame around text.
    :param text:int  The text to frame.
    :args:int  Additional text to frame.(arg per line)
    :param padding_horizontal: int The amount of padding to add horizontally. Halved so 4 = 2 on each side.
    :param padding_vertical:  int The amount of padding to add vertically. Halved so 4 = 2 on each side.
    returns: str The framed text.
    ...
    top_row = f"{frame.top_left}{frame.top * width}{frame.top_right}"
    rows = [top_row]
    pad_row = frame.right + (" " * width) + frame.left 
    horizontal_padding = [pad_row for i in range((padding_horizontal//2) )] #make the padding rows(added before and after text)
    rows.extend(horizontal_padding)
    # now add the text
    for line in lines:
        line = line.center(width, " ") #center the text in the frame
        line = frame.left +  line  + frame.right #add the left and right frame elements
        rows.append(line)
    # now add the bottom rows
    
    rows.extend(horizontal_padding)
    # make the bottom row
    rows.append( f"{frame.bottom_left}{frame.bottom * width}{frame.bottom_right}")
    return "\n".join(rows)